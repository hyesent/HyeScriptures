...
import { Plus, MessageSquare, Heart, Users, Filter, TrendingUp, Clock, RefreshCw } from 'lucide-react'
...
export const CommunityTab: React.FC<CommunityTabProps> = ({ prefillVerse, onClearPrefill }) => {
  ...
  const filterPosts = () => {
    ...
    if (activeTab === 'prayer') {
      filtered = filtered.filter(p => p.post_type === 'prayer_request')
    } else if (activeTab === 'questions') {
      filtered = filtered.filter(p => p.post_type === 'bible_question')
        ...
        <div className={styles.tabs}>
          {tabs.map((tab) => {
            ...
            const count = tab.id === 'feed' ? posts.length : 
                         ...
                         tab.id === 'questions' ? posts.filter(p => p.post_type === 'bible_question').length :
                         0
            return (
              <button
                key={tab.id}
        ...
        <div className={styles.sortOptions}>
          {sorts.map((sort) => {
            ...
            return (
              <button
                key={sort.id}
                className={`${styles.sortBtn} ${activeSort === sort.id ? styles.active : ''}`}
        ...
        <div className={styles.feed}>
          {filteredPosts.length === 0 ? (
            <div className={styles.emptyState}>
              ...
              <button className={styles.emptyBtn} onClick={() => setShowEditor(true)}>
                <Plus size={16} />
                Create Post
              </button>
            </div>
