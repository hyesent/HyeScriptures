...
export const SermonBuilder: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError('Error generating sermon. Please check your connection and try again.')
    } finally {
      setLoading(false)
    }
  }
...
const callSermonEdgeFunction = async (prompt: string): Promise<any> => {
  try {
    ...
    if (!data || !data.response) {
      console.error('No response data from sermon function:', data)
      return null
    }
    return data.response
  } catch (err) {
    console.error('Sermon function exception:', err)
    return null
  }
}
